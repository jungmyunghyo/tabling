function fn_make_table_roulette(tp, ran) {
	$("#btn--table-roulette--cls").attr("disabled", false);
	$("#btn--table-roulette--strt").attr("disabled", false);
	$("#canvas-table-roulette").empty();
	var cols = ["#93DAFF", "#3DFF92", "#FFB6C1"];
	var canvas = document.querySelector("#canvas-table-roulette");
	if (ran == 9999) {
...
function fn_rotate_table_roulette(tp) {
	$("#btn--table-roulette--cls").attr("disabled", true);
	$("#btn--table-roulette--strt").attr("disabled", true);
	var canvas = document.querySelector("#canvas-table-roulette");
	canvas.style.transform = "initial";
	canvas.style.transition = "initial";
	var prds = fn_make_table_roulette_prd(tp);
...
function fn_make_user_roulette(ran) {
	$("#btn--user-roulette--cls").attr("disabled", false);
	$("#btn--user-roulette--strt").attr("disabled", false);
	$("#canvas-user-roulette").empty();
	var cols = ["#93DAFF", "#3DFF92", "#FFB6C1"];
	...
	var canvas_cw = Number(canvas.width / 2);
	var canvas_ch = Number(canvas.height / 2);
	var prds = fn_make_arr($("#btn--user-roulette--strt").attr("arr"));
	var prdCnt = Number(prds.length);
	var arc = Number((2 * Math.PI) / prdCnt);
	for (var i=0; i<prdCnt; i++) {
		canvas_ctx.beginPath();
